...
export type NormalizedSelectOptions<Options extends readonly SelectOptionConfigInput[]> = Readonly<NormalizedTuple<Options>> extends infer Result
  ? Result extends readonly SelectOptionInput[]
    ? Result
    : never
  : never;

export type SelectOptionUnion<
  TOptions extends readonly SelectOptionInput[] | undefined,
  TAllowCustom extends boolean
> = TAllowCustom extends true
  ? SelectOptionInput
  : TOptions extends readonly SelectOptionInput[]
    ? TOptions[number]
    : SelectOptionInput;

export function normalizeSelectOptions<const Options extends readonly SelectOptionConfigInput[]>(
  options: Options
...
export function buildOptionConfig(property: "select" | "multi_select", options?: readonly SelectOptionInput[]): ((name: string) => Record<string, unknown>) | undefined {
